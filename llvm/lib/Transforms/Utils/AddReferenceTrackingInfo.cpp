...
void llvm::setReferenceInfo(Instruction *I, const std::string &Name,
  ...
  I->setMetadata(LLVMContext::MD_memtracer, MD);
}

bool llvm::getReferenceInfo(const StoreInst *SI, std::string &Name,
                            std::string &TypeName) {
  MDNode *MD = SI->getMetadata(LLVMContext::MD_memtracer);
...
static ReferenceDbgInfo getReferenceDbgInfoFromGlobalVar(GlobalVariable *GV) {
  ...
  if (!GVEs.empty()) {
    if (DIGlobalVariable *GVar = GVEs.front()->getVariable()) {
...
// Collect Value -> ReferenceDbgInfo from dbg.declare (intrinsic or
// DbgVariableRecord), and count store ptr,ptr.
...
static unsigned collectPtrToDbgVarAndStorePtrCount(
  ...
  for (BasicBlock &BB : F) {
    for (Instruction &I : BB) {
      // New debug format: DbgVariableRecord declare attached to instructions.
      for (DbgVariableRecord &DVR : filterDbgVars(I.getDbgRecordRange())) {
        if (!DVR.isDbgDeclare())
          continue;
        Value *Addr = DVR.getVariableLocationOp(0);
        if (Addr && DVR.getVariable())
          PtrToDbgVar.try_emplace(
              Addr, getReferenceDbgInfoFromVariable(DVR.getVariable()));
      }
  ...
  for (BasicBlock &BB : F) {
    for (Instruction &I : BB) {
      ...
      if (auto *SI = dyn_cast<StoreInst>(&I)) {
        ...
        if (SI->getValueOperand()->getType()->isPointerTy())
          if (MDNode *MD = SI->getMetadata(LLVMContext::MD_memtracer))
  ...
  for (BasicBlock &BB : F) {
    for (Instruction &I : BB) {
      ...
      if (It == StorePtrToMemtracerMD.end() || !It->second)
        continue;
      CB->setMetadata(LLVMContext::MD_memtracer, It->second);
      Changed = true;
    }
  }
  return Changed;
