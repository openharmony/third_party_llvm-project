...
bool CrashSnapshotManager::parseStackFrameLine(llvm::StringRef Line,
  ...
  if (!Line.startswith(SF.LinePrefix))
    return false;
  ...
  if (LastToken.startswith(Strip.Open) && LastToken.endswith(Strip.Close)) {
...
bool CrashSnapshotManager::loadFromLogAndDump(const std::string &LogFilePath) {
  ...
  for (llvm::line_iterator LineIt(*BufferOrErr.get(), true);
    ...
    if (State == FAULT_THREAD && Line.startswith(Cfg.StackFrame.LinePrefix) &&
        CurrentThread) {
      uint32_t FrameIdx;
      uint64_t PC;
      std::string ModuleName;
