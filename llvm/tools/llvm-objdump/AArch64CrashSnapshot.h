...
public:
  ...
  llvm::Optional<std::string> getMemoryValue(uint64_t Addr,
                                             uint64_t Size) const {
    if (Size == 0)
      return llvm::None;
  ...
  llvm::Optional<uint64_t> getRegister(uint16_t DwarfRegNum) const {
  ...
  llvm::Optional<uint64_t> getCleanPC() const {
    auto Val = getRegister(32);
    return Val ? llvm::Optional<uint64_t>(StripPAC(*Val)) : llvm::None;
  }

  llvm::Optional<uint64_t> getCleanSP() const {
