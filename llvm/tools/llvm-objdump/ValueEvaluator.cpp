...
ValueEvaluator::evaluate(DWARFDie VarDie, ArrayRef<uint8_t> ExprData,
  ...
  llvm::Optional<uint64_t> PendingValue = llvm::None;
  ...
  for (auto &Op : Expr) {
    ...
    if (Opcode == dwarf::DW_OP_entry_value ||
      ...
      llvm::Optional<uint16_t> TargetReg;
      ...
      llvm::Optional<uint64_t> RecoveredVal = llvm::None;
  ...
  while (TypeDie.isValid() && MaxDepth-- > 0) {
    ...
    if (Tag == dwarf::DW_TAG_base_type) {
      ...
      if (auto TypeNameOpt = dwarf::toString(TypeDie.find(dwarf::DW_AT_name))) {
        ...
        if (TypeName.equals("char") || TypeName.equals("unsigned char") ||
            TypeName.equals("signed char"))
          return 1;
        if (TypeName.equals("short") || TypeName.equals("unsigned short"))
          return 2;
        ...
        if (TypeName.equals("float"))
          return 4;
        if (TypeName.equals("double") || TypeName.equals("long long"))
          return 8;
      }
...
// operation fails, llvm::None returned.
