...
; RUN-NOT: llc < %s -mtriple aarch64-linux-gnu -mattr=+pauth                  -verify-machineinstrs -disable-post-ra \
; RUN-NOT:   -global-isel=1 -global-isel-abort=1 -o - %s | FileCheck %s

; RUN: llc < %s -mtriple aarch64-linux-gnu -mattr=+pauth,+pauth-hint-only -verify-machineinstrs -disable-post-ra \
; RUN:    -global-isel=0 -o - %s | FileCheck %s --check-prefixes=CHECK,HINT
; RUN-NOT: llc < %s -mtriple aarch64-linux-gnu mattr=+pauth,+pauth-hint-only -verify-machineinstrs -disable-post-ra \
; RUN-NOT:   -global-isel=1 -global-isel-abort=1 -o - %s | FileCheck %s
