...
// RUN: %clang %flags -DTOOL -shared -fPIC %s -o %T/libntool.so %send-libntool
...
// RUN: %libomp-compile -DCODE -L%T %no-as-needed-flag -lntool && \
// RUN:    env OMP_TOOL_VERBOSE_INIT=stdout LD_LIBRARY_PATH=$LD_LIBRARY_PATH:%tool-path %libomp-run | \
// RUN:    FileCheck %s --check-prefixes CHECK,ADDRSPACE

// 2.2 Link with tool during compilation, but AFTER the runtime

// RUN: %libomp-compile -DCODE -L%T -lomp %no-as-needed-flag -lntool && \
...
// RUN:    env LD_PRELOAD=%tool-path/libntool.so %libomp-run | FileCheck %s \
// RUN:    --check-prefixes CHECK,ADDRSPACE
...
// RUN: %libomp-compile -DCODE && env OMP_TOOL_LIBRARIES=%tool-path/libntool.so \
...
int main()
{
  ...
  // TOOLLIB-NEXT: OMP_TOOL_LIBRARIES = [[PARENTPATH]]/libntool.so
  // TOOLLIB-NEXT: Opening [[PARENTPATH]]/libntool.so... Success.
  // TOOLLIB-NEXT: Searching for ompt_start_tool in
  // TOOLLIB-SAME: [[PARENTPATH]]/libntool.so...
