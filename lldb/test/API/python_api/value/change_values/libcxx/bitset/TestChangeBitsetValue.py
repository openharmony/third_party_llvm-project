"""
Test change libc++ std::bitset values.
"""
...
class LibcxxChangeBitsetValueTestCase(TestBase):
    ...
    def do_test_var(self, var_name, frame, expected_child_cnt):
        ...
        self.assertTrue(
            child_cnt == expected_child_cnt,
            "Expected child cnt = {}".format(expected_child_cnt),
        )
        for i in range(child_cnt):
            child_val = value.GetChildAtIndex(i)
            self.assertTrue(
                child_val.IsValid(), "Got the SBValue for inner bitset chunk"
            )
            new_val = 1000 + i
            result = child_val.SetValueFromCString(str(new_val))
            self.assertTrue(result, "Setting val returned True.")
            self.assertTrue(
                child_val.GetValueDidChange(), "LLDB noticed that value changed"
            )
            result = child_val.GetValueAsUnsigned()
            self.assertTrue(
                result == new_val, "Got correct value ({})".format(new_val)
            )
