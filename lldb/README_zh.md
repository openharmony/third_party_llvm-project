...
LLDB（Low Lever Debugger）是新一代高性能调试器。当前Openharmony主干分支的LLDB工具是在[llvm21.1.8](https://github.com/llvm/llvm-project/releases/tag/llvmorg-21.1.8)基础上适配演进出来的工具。支持在桌面和Openharmony设备或模拟器上调试。
...
通过OpenHarmony的SDK获取，获取路径：[每日构建](http://ci.openharmony.cn/dailys/dailybuilds) 。
...
>
> lldb-server所在路径会根据llvm版本变更有所变化。比如12.0.1版本的llvm，lldb-server所在目录则在\***\native\llvm\lib\clang\12.0.1\bin；21.1.8版本的llvm，lldb-server所在目录则在*\**\native\llvm\lib\clang\21.1.8\bin。
...
2）Install路径：llvm-project/out/llvm-install/lib/clang/21.1.8/bin/[platform]-linux-ohos

3）package压缩包：llvm-project/out/clang-dev-linux-x86_64.tar.bz2内，clang-dev/lib/clang/21.1.8/bin/[platform]-linux-ohos
