...
Status HdcClient::CreateByDeviceID(const std::string &device_id,
  ...
  if (hdc_utid.empty()) {
    if (connect_devices.size() != 1)
      return Status::FromErrorStringWithFormat("Expected a single connected device, got instead %zu - try "
                    "setting 'HDC_UTID'",
...
Status HdcClient::Connect() {
  ...
  if (m_device_id.size() > sizeof(handshake.connectKey))
    return Status::FromErrorStringWithFormat("Device id is too long: %s", m_device_id.c_str());
  ...
  if (env_port != NULL) {
    ...
    if ((iEnv_port > 0) && (iEnv_port <= 65535)) {
      ...
      return Status::FromErrorStringWithFormat("invalid port specification: %s. $OHOS_HDC_SERVER_PORT must be a positive number in (0,65535]", env_port);
    }
  }
...
Status HdcClient::SetPortForwarding(const uint16_t local_port,
  ...
  Status error = SendMessage(message);
  if (error.Fail())
    return error;
...
Status HdcClient::LocalTransferFile(const char *direction, const FileSpec &src,
  ...
  if (!hdc_tool) {
      ...
      return Status::FromErrorStringWithFormat("HDC tool not available");
  }
  ...
  std::string errorMsg;
  result = llvm::sys::ExecuteAndWait(hdc_tool.get(), hdc_args, std::nullopt, {},
                                     0, 0, &errorMsg);
  if (result != 0) {
      LLDB_LOGF(log, "HdcClient::LocalTransferFile Execute failed : %s",
                errorMsg.c_str());
      return Status::FromErrorStringWithFormat("HdcClient::LocalTransferFile failed execute cmd");
  }
...
Status HdcClient::ExpectCommandMessagePrefix(uint16_t expected_command,
  ...
  if (command != expected_command)
    return Status::FromErrorStringWithFormat("Unexpected HDC server command: %d, expected %d", command,
                  expected_command);
...
struct HdcTagIO {
  ...
  static std::optional<std::pair<size_t, WireType>> ParseTag(InItT &InBegin,
                                                              InItT InEnd) {
    std::optional<U> Tag = HdcIO<U>::Parse(InBegin, InEnd);
    if (!Tag)
      return std::nullopt;
    U Val = *Tag;
...
struct HdcIOBase {
  ...
  static std::optional<T> ParseTagged(size_t Idx, InItT &InBegin,
                                       InItT InEnd) {
    std::optional<std::pair<size_t, HdcTagIO::WireType>> Tag =
        HdcTagIO::ParseTag(InBegin, InEnd);
    if (!Tag || Tag->first != Idx || Tag->second != TheType)
      return std::nullopt;
...
struct HdcIO : HdcIOBase<HdcTagIO::WireType::VARINT, T, HdcIO<T>> {
  ...
  template <typename InItT>
  static std::optional<T> Parse(InItT &InBegin, InItT InEnd) {
...
struct HdcIO<std::string> : HdcIOBase<HdcTagIO::WireType::LENGTH_DELIMETED,
  ...
  template <typename InItT>
  static std::optional<std::string> Parse(InItT &InBegin, InItT InEnd) {
...
template <size_t StartIdx, typename... Ts> struct HdcTaggedIOHelper {
  template <typename InItT>
  static std::optional<std::tuple<>> Parse(InItT &InBegin, InItT InEnd) {
    return std::tuple<>{};
  }
...
struct HdcTaggedIOHelper<StartIdx, T, Ts...> {
  using ResT = std::optional<std::tuple<T, Ts...>>;

  template <typename InItT> static ResT Parse(InItT &InBegin, InItT InEnd) {
    std::optional<T> LHS = HdcIO<T>::ParseTagged(StartIdx, InBegin, InEnd);
    if (!LHS)
      return std::nullopt;
    std::optional<std::tuple<Ts...>> RHS =
        HdcTaggedIOHelper<StartIdx + 1, Ts...>::Parse(InBegin, InEnd);
    if (!RHS)
      return std::nullopt;
    return std::tuple_cat(std::make_tuple(std::move(*LHS)), std::move(*RHS));
  }
...
Status HdcClient::FileCheck(int FD, size_t &file_size) {
  ...
  if (!maybe_file_mode)
    return Status::FromErrorStringWithFormat("Could not parse HDC server FileMode");
  ...
  if (EC)
    return Status::FromErrorStringWithFormat("%s", EC.message().c_str());
  ...
  if (!transfer_config.has_value())
    return Status::FromErrorStringWithFormat("Could not parse HDC server TransferConfig");

  if (auto compress_type = std::get<7>(*transfer_config))
    return Status::FromErrorStringWithFormat("Compression is not supported");
...
Status HdcClient::PullFileChunk(std::vector<char> &buffer) {
  ...
  if (!transfer_payload.has_value())
    return Status::FromErrorStringWithFormat("Could not parse HDC server TransferPayload");
...
Status HdcClient::RecvFile(const FileSpec &src, const FileSpec &dst) {
  ...
  if (EC)
    return Status::FromErrorStringWithFormat("Unable to open local file %s", local_file_path.c_str());
  ...
  if (dst_file.has_error())
    return Status::FromErrorStringWithFormat("Failed to write file %s", local_file_path.c_str());
...
Status HdcClient::SendFile(const FileSpec &src, const FileSpec &dst) {
  ...
  if (src_file.bad())
    return Status::FromErrorStringWithFormat("Failed read on %s", local_file_path.c_str());
  if (sent_bytes < status.getSize()) {
    m_conn.reset();
    return Status::FromErrorStringWithFormat("Failed to read all of the bytes from %s: read %zu/%zu",
...
Status HdcClient::ReadCommandMessagePrefix(uint16_t &command, std::vector<char> &message, size_t prefix_size) {
  ...
  if (packet_len < sizeof(command))
    return Status::FromErrorStringWithFormat("Message too small to contain a command");
...
Status HdcClient::ReadMessageStream(std::vector<char> &message,
  ...
  while (error.Success() && status == lldb::eConnectionStatusSuccess) {
    ...
    if (elapsed >= timeout)
      return Status::FromErrorStringWithFormat("Timed out");
...
Status HdcClient::ReadResponseStatus(const char *expected) {
  ...
  if (expected == nullptr ||
    ...
    return Status::FromErrorStringWithFormat("%s", message.data());
