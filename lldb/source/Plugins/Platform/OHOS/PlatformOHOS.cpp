...
Status PlatformOHOS::ConnectRemote(Args &args) {
  ...
  if (IsHost()) {
    return Status::FromErrorStringWithFormat("can't connect to the host platform '%s', always connected",
...
Status PlatformOHOS::DownloadModuleSlice(const FileSpec &src_file_spec,
  ...
  if (src_offset != 0)
    return Status::FromErrorStringWithFormat("Invalid offset - %" PRIu64, src_offset);
