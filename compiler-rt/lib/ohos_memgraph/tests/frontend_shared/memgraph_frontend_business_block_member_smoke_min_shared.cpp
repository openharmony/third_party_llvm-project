//===-- memgraph_frontend_business_block_member_smoke_min_shared.cpp ===//
...
memgraph_frontend_business_block_member_smoke_min_run() {
  // Explicitly call alloc_record()/store_record() and then verify block/member
  // query results.
  auto alloc_record =
  ...
  block_info_t block = {};
  if (!get_block_info((unsigned long)raw, &block) ||
      block.base != (unsigned long)raw || block.size != sizeof(Node) ||
      block.malloc_pc == 0)
  ...
  const unsigned long expected_alloc_pc =
      (unsigned long)(void *)&memgraph_frontend_business_block_member_smoke_min_run;
  alloc_record((unsigned long)raw, "Node*", "node", expected_alloc_pc);
  if (!get_block_info((unsigned long)raw, &block) ||
      (!block.type_name && !block.name) ||
  ...
  printf(
      "PASS: base=0x%lx size=%lu malloc_pc=0x%lx block_type=%s block_name=%s "
      "member_type=%s member_name=%s\n",
      block.base, block.size, block.malloc_pc,
      block.type_name ? block.type_name : "<null>",
