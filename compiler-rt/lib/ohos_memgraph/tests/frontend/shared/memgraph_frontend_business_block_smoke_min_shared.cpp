//===-- memgraph_frontend_business_block_smoke_min_shared.cpp -----===//
...
memgraph_frontend_business_block_smoke_min_run() {
  // Explicitly call alloc_record() and then verify block-level metadata can be
  // queried.
  auto alloc_record =
  ...
  if (after.type_name || after.name)
    return fprintf(stderr,
                   "unexpected metadata before manual alloc_record: type=%s "
                   "name=%s\n",
  ...
  const unsigned long expected_alloc_pc =
      (unsigned long)(void *)&memgraph_frontend_business_block_smoke_min_run;
