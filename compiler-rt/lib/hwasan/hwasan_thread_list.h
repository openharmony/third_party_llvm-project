 ...
 public:
  ...
  void AddFreedRingBuffer(Thread *t) {
    ...
    for (uptr i = 0, size = rb->realsize(); i < size; i++) {
      // Copy complete HAR; source only pushed 4 fields.
      freed_allocations_->push((*rb)[i]);
    }
  ...
  // ThreadList-level tracing gate (fallback deallocate path). Coexists with
  // Thread::AllowTracingHeapAllocation from prior OHOS ports.
