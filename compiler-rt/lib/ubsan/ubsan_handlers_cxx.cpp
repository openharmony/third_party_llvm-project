...
static bool EndsWith(const char *Str, uptr StrLen, const char *Pattern,
                     uptr PatternLen) {
...
static const char *UndecorateCfiTypeIdSymbol(char *Str) {
  ...
  const uptr StartPatternLen = sizeof(StartPattern) - 1;
  const uptr EndPatternLen = sizeof(EndPattern) - 1;
...
void __ubsan_handle_cfi_bad_type(CFICheckFailData *Data, ValueHandle Vtable,
  ...
  if (DTI.isValid()) {
    ...
    if (ValidVtable) {
      ...
      if (const char *Undecorated =
              UndecorateCfiTypeIdSymbol(VtableDataInfo.name))
