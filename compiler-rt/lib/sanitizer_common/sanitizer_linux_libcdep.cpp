  ...
  if (!main) {
    ...
    if (*tls_begin > *stk_begin && *tls_begin < *stk_end) {
      ...
      *stk_end = *tls_begin;
      ...
      // 实现注释中逻辑：调整TLS和栈的范围，让TLS紧贴栈尾
      uptr original_tls_end = *tls_begin + tls_size;  // TLS原始结束地址（GetTls得到的完整范围）
      uptr current_stk_end = *stk_end;                // 栈当前结束地址（栈尾）

      if (current_stk_end < original_tls_end) {
        // 场景1：TLS原始结束地址超出栈尾 → TLS部分重叠/超出栈
        // 新TLS起始 = 栈尾，新TLS大小 = 原始TLS结束 - 栈尾（仅保留栈外的TLS部分）
        *tls_begin = current_stk_end;
        tls_size = original_tls_end - current_stk_end;
      } else {
        // 场景2：TLS原始结束地址未超出栈尾 → TLS完全重叠在栈内
        // 新TLS大小 = 栈尾 - TLS原始起始（保留栈内的TLS部分）
        // 栈缩小：栈结束地址 = TLS原始起始（栈和TLS衔接，不重叠）
        tls_size = current_stk_end - *tls_begin;
        *stk_end = *tls_begin;
      }
