 ...
 */
 ...
 static int gwp_asan_current_frame_in_ffrt_stack(void)
 {
     ...
     if (!g_ffrt.get_current_coroutine_stack(&stack_addr, &stack_size)) {
         fprintf(stderr, "[gwp_asan_ffrt_dlopen_coroutine_uaf] FAIL: coroutine stack unavailable.\n");
         return 0;
     }
     if (stack_addr == NULL || stack_size == 0) {
         fprintf(stderr, "[gwp_asan_ffrt_dlopen_coroutine_uaf] FAIL: empty coroutine stack range.\n");
         return 0;
     }
 
     uintptr_t stack_start = (uintptr_t)stack_addr;
     if (stack_start > UINTPTR_MAX - stack_size) {
         fprintf(stderr, "[gwp_asan_ffrt_dlopen_coroutine_uaf] FAIL: coroutine stack range overflows.\n");
         return 0;
     }
     uintptr_t stack_end = stack_start + stack_size;
     uintptr_t frame = (uintptr_t)__builtin_frame_address(0);
     if (frame < stack_start || frame >= stack_end) {
         fprintf(stderr, "[gwp_asan_ffrt_dlopen_coroutine_uaf] FAIL: frame %p is not in FFRT stack [%p, %p).\n",
                 (void *)frame, (void *)stack_start, (void *)stack_end);
         return 0;
     }
     return 1;
 }
 ...
 static void gwp_asan_ffrt_dlopen_coroutine_task(void *arg)
 {
     ...
     if (!gwp_asan_check_sampled_dynamic(ptr, "gwp_asan_ffrt_dlopen_coroutine_uaf")) {
         free(ptr);
         exit(4);
     }
 ...
 int main(void)
 {
     const char *case_name = "gwp_asan_ffrt_dlopen_coroutine_uaf";
