...
void level30_trigger_error(int param, char *buf, int *arr);
...
void level29_function_bb(int param, char *buf, long long *arr) {
  ...
  level30_trigger_error(local_var29, buffer29, array29);
}
...
void level30_trigger_error(int param, char *buf, int *arr) {
  ...
  // Local variables
  int local_var30 = param & 0x1E1E1E1E;
  char buffer30[10];  // Small buffer to trigger overflow
  char *volatile p = buffer30;
  
  // This will cause a tag mismatch error - out of bounds access
  // Accessing beyond the buffer size will trigger HWASan error
  p[15] = 0;  // Out of bounds access - should trigger error
}
