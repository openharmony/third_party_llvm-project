...
# OHOS_LOCAL begin: remote_interface dispatch (hdc/adb) when configured.
_remote_interface = getattr(config, "remote_interface", "") or ""
if _remote_interface:
    remote_cmd = None

    if _remote_interface == "hdc":
        for var in (
            "HDC",
            "HDC_SERVER_IP_PORT",
            "HDC_UTID",
        ...
        config.substitutions.append(
            ("%push_to_device", remote_interface.config_push_str)
        )
        config.substitutions.append(
            ("%adb_shell ", remote_interface.config_shell_str)
        )
        config.substitutions.append(
            ("%device_rm", remote_interface.config_remove_str)
        )
        remote_cmd = remote_interface.hdc_output
        push_cmd = remote_interface.push
        connect = remote_interface.connect
    elif _remote_interface == "adb":
    ...
    if config.android:
        ...
        config.substitutions.append(
            ("%device_rundir/", android_common.ANDROID_TMPDIR.rstrip("/") + "/")
        )
        try:
            android_api_level_str = remote_cmd(
                ["shell", "getprop", "ro.build.version.sdk"], env=env
            ).rstrip()
            android_api_codename = (
                remote_cmd(
                    ["shell", "getprop", "ro.build.version.codename"], env=env
                )
                .rstrip()
                .decode("utf-8")
            )
            ...
            lit_config.fatal(
                "Failed to read ro.build.version.sdk (using '%s' as adb)"
                % remote_interface.command
            )
        try:
        ...
        # Prepare the device.
        remote_cmd(
            ["shell", "mkdir", "-p", android_common.ANDROID_TMPDIR], env=env
        )
        ...
        if config.ohos_host:
            ...
            config.substitutions.append(
                ("%device_rundir/", ohos_common.TMPDIR.rstrip("/") + "/")
            )
            connect()
            remote_cmd(
                ["shell", "mkdir", "-p", ohos_common.TMPDIR], env=env
            )
    else:
        config.substitutions.append(("%device_rundir/", ""))
        config.substitutions.append(("%push_to_device", "echo "))
