...
private:
  ...
  bool isHeapObject(SVal Val, ProgramStateRef State) const {
    /**
    ...
    if (Region) {
      const MemSpaceRegion *Space = Region->getMemorySpace(State);
...
void NotHeapObjChecker::checkPreCall(const CallEvent &Call,
  ...
  if ((dyn_cast<AnyCXXConstructorCall>(&Call) &&
    ...
    if (!isHeapObject(Val, C.getState())) { // if the object is not on the heap, report bugs
      reportBug(Call, C);
    }
  }
}
