...
NapiScopeManagerChecker::NapiScopeManagerChecker()
    : OpenHandleScope(CallDescription::Mode::SimpleFunc,
                      {"napi_open_handle_scope"}, NAPI_SCOPE_AYGS_NUM),
      CloseHandleScope(CallDescription::Mode::SimpleFunc,
                       {"napi_close_handle_scope"}, NAPI_SCOPE_AYGS_NUM) {
...
void NapiScopeManagerChecker::checkPreCall(const CallEvent &Call,
                                           CheckerContext &C) const {
  if (!CloseHandleScope.matches(Call)) {
    return;
  }
  ProgramStateRef State = C.getState();
...
void NapiScopeManagerChecker::checkPostCall(const CallEvent &Call,
                                            CheckerContext &C) const {
  if (!OpenHandleScope.matches(Call)) {
    return;
  }
  ProgramStateRef State = C.getState();
  ...
  std::optional<loc::MemRegionVal> X = ScopeSVal.getAs<loc::MemRegionVal>();
  ...
  std::optional<DefinedOrUnknownSVal> Dval = StatusVal.getAs<DefinedOrUnknownSVal>();
  if (!Dval) {
    return;
  }
