...
// functions within signal handler function
//
//===----------------------------------------------------------------------===//
...
void ento::registerSignalHandlerChecker(CheckerManager &mgr) {
    ...
    std::optional<functionConfig> obj = getConfiguration<functionConfig>(mgr, Checker, Option, ConfigFile);
    if (obj) {
        Checker->parseConfiguration(mgr, Option, &obj.value());
    }
}
