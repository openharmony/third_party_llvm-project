...
NapiGetArrBufferDataChecker::NapiGetArrBufferDataChecker()
    : NapiGetArrBuffer(CallDescription::Mode::SimpleFunc,
                       {"napi_get_arraybuffer_info"},
                       NAPI_GET_ARRAYBUFFER_INFO_AYGS_NUM),
      Free(CallDescription::Mode::CLibrary, {"free"}, FREE_AYGS_NUM) {
...
void NapiGetArrBufferDataChecker::checkPostCall(const CallEvent &Call,
  ...
  if (!NapiGetArrBuffer.matches(Call)) {
    return;
  }
  ProgramStateRef State = C.getState();
  SVal S = Call.getArgSVal(NAPI_DATA_POS);
  std::optional<loc::MemRegionVal> X = S.getAs<loc::MemRegionVal>();
  if (!X)
    return;
